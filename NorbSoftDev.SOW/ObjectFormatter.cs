    ...
    public static class ObjectFormatter
    {
        ...
        private static object GetPropertyValueFromPath(object target, string path)
        {
            ...
            return retVal;
                    ...
                    Log.Info(target,"[GetPropertyValueFromPath] Target: "+target+" path:"+path);
                    throw;
            }
        }
        ...
        private static object GetPropertyValue(object target, string name, string indexList)
        {
            ...
            if (target == null) {
                Log.Info(target,"[GetPropertyValue] Target: null name:"+name+" indexList:"+indexList);
                throw new Exception("Target is null "+name+" "+indexList);
            }
            ...
            if (property == null) {
                Log.Error( target, string.Format("Unable to find the property '{0}'' in type '{1}'", name, target.GetType().FullName) );
